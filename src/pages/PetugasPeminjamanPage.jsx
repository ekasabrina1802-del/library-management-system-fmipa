...
function BookCoverSmall({ bookCode, imageUrl, title }) {
  ...
  return (
    <div style={{ width: 40, height: 54, borderRadius: 4, overflow: 'hidden', flexShrink: 0, background: '#eee' }}>
      {imageUrl
        ? <ApiImage src={imageUrl} alt={title} style={{ width: '100%', height: '100%', objectFit: 'cover' }}
            fallback={<div style={{ width: '100%', height: '100%', background: COVER_COLORS[prefix] || '#555', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 8, fontWeight: 700 }}>{prefix}</div>} />
        : <div style={{ width: '100%', height: '100%', background: COVER_COLORS[prefix] || '#555', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 8, fontWeight: 700 }}>{prefix}</div>
      }
    </div>
  );
}
    ...
    <div className="modal-overlay">
      <div className="modal" style={{ maxWidth: 520, width: '95%', padding: 0, overflow: 'hidden', borderRadius: 16, boxShadow: '0 24px 60px rgba(0,0,0,0.18)' }}>
        ...
        <div style={{
          ...
          padding: '18px 22px 16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between',
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <BookOpen size={16} style={{ color: 'rgba(255,255,255,0.8)' }} />
            <span style={{ color: 'white', fontWeight: 700, fontSize: 15 }}>Detail Peminjaman</span>
          </div>
        ...
        <div style={{ padding: '20px 22px 22px' }}>
          {/* Book Info */}
          <div style={{ display: 'flex', gap: 14, padding: '0 0 16px', borderBottom: '1px solid #f0ebe6', marginBottom: 16 }}>
          ...
          {/* Detail rows */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, border: '1px solid #f0ebe6', borderRadius: 10, overflow: 'hidden', marginBottom: 16 }}>
            {[
              ['Peminjam', <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ fontWeight: 700 }}>{loan.memberName}</span><span className="badge badge-info" style={{ fontSize: 10 }}>{loan.memberType}</span></span>],
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button
              ...
              style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, opacity: bisaExt ? 1 : 0.45, cursor: bisaExt ? 'pointer' : 'not-allowed' }}
            >
            ...
            <button
              ...
              style={{ flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'linear-gradient(135deg,#2D6A4F,#40916C)' }}
            >
...
export default function PetugasPeminjamanPage() {
  ...
  const validate = (member, bookCode) => {
    ...
    const copy =
      book.copies?.find(c => c.copy_code?.toLowerCase().includes(bookCode.toLowerCase()) && c.status === 'available') ||
      book.copies?.find(c => c.status === 'available');
    if (!copy) return { valid: false, msg: `Tidak ada eksemplar/copy tersedia untuk "${book.title}".` };
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await addLoan({ memberId: member.id, bookId: v.book.id, copyId: v.copy.id, copyCode: v.copy.copy_code });
    if (res.success) {
  ...
  const handleExtend = async (loan) => {
    ...
    const rule = RULES[loan.memberType?.toLowerCase() || 'mahasiswa'] || RULES.mahasiswa;
  ...
  const totalActive = processedLoans.filter(l => ['dipinjam', 'terlambat', 'diperpanjang'].includes(l.status)).length;

  const previewMember = nimInput ? findMember(nimInput.trim()) : null;
  const previewBook   = bookCodeInput
    ? books.find(b => b.title?.toLowerCase().includes(bookCodeInput.trim().toLowerCase()))
    : null;
  ...
  return (
    <div>
      {selectedLoan && (
        <LoanDetailModal
          loan={selectedLoan}
          book={books.find(b => b.id === selectedLoan.bookId || b.no_induk === selectedLoan.bookCode || b.copies?.some(c => c.copy_code === selectedLoan.bookCode))}
      ...
      {/* ── Main Area: Form (top) + Table (below) ── */}

      {/* Form Card — full width, horizontal layout */}
      <div className="card" style={{ padding: 0, overflow: 'hidden', marginBottom: 20 }}>
        {/* Card header */}
        <div style={{
          padding: '14px 22px', borderBottom: '1px solid #f0ebe6',
          background: 'linear-gradient(to right, #fafafa, #fff5f5)',
          display: 'flex', alignItems: 'center', gap: 10,
        }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: 'rgba(123,28,28,0.08)', border: '1px solid rgba(123,28,28,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Plus size={15} style={{ color: '#7B1C1C' }} />
          </div>
          <div>
            <div style={{ fontWeight: 700, fontSize: 14, color: '#1a1a1a', lineHeight: 1.2 }}>Form Peminjaman Baru</div>
            <div style={{ fontSize: 11, color: '#9ca3af', marginTop: 2 }}>Masukkan NIM/NIP anggota dan cari buku yang ingin dipinjam</div>
          </div>
        </div>
        ...
        <div style={{ padding: '18px 22px' }}>
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 16, alignItems: 'start' }}>
              ...
              <div>
                ...
                {nimInput && (previewMember ? (
                  <div style={{ fontSize: 11, marginTop: 6, padding: '6px 10px', background: 'rgba(46,125,50,0.07)', border: '1px solid rgba(46,125,50,0.2)', borderRadius: 6, color: '#2E7D32' }}>
                    <div style={{ fontWeight: 700 }}>✓ {previewMember.name}</div>
                    <div style={{ marginTop: 2, color: '#4b7c5a' }}>
                      {previewMember.type} · {getActiveLoanCount(previewMember.id)}/{RULES[previewMember.type?.toLowerCase()]?.maxBuku || '?'} buku aktif
                    </div>
                  </div>
                ) : (
                  <div style={{ fontSize: 11, color: '#B71C1C', marginTop: 6, padding: '5px 10px', background: 'rgba(183,28,28,0.06)', borderRadius: 6 }}>
              ...
              <div>
                <label className="form-label" style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 6 }}>
                  <BookOpen size={12} /> Nama / Kode Buku
                </label>
                <div style={{ position: 'relative' }}>
                  <Search size={13} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: '#9ca3af' }} />
                  <input
                ...
                {bookCodeInput && (previewBook ? (
                  <div
                    ...
                    style={{
                      fontSize: 11, marginTop: 6, padding: '6px 10px', borderRadius: 6, cursor: 'pointer',
                      background: previewBook.available > 0 ? 'rgba(46,125,50,0.07)' : 'rgba(183,28,28,0.07)',
                      border: `1px solid ${previewBook.available > 0 ? 'rgba(46,125,50,0.2)' : 'rgba(183,28,28,0.2)'}`,
                      color: previewBook.available > 0 ? '#2E7D32' : '#B71C1C',
                    }}
                  >
                    <div style={{ fontWeight: 700 }}>✓ {previewBook.title}</div>
                    <div style={{ marginTop: 2 }}>
                      {previewBook.available > 0 ? `Tersedia ${previewBook.available} eksemplar · Klik untuk memilih` : 'Stok habis'}
                    </div>
                  </div>
                ) : (
                  <div style={{ fontSize: 11, color: '#B45309', marginTop: 6, padding: '5px 10px', background: 'rgba(180,83,9,0.06)', borderRadius: 6 }}>
                    ✗ Buku tidak ditemukan
                  </div>
                ))}
              </div>

              {/* Submit button — vertically centered with inputs */}
              <div style={{ paddingTop: 24 }}>
                <button
                  type="submit"
                  className="btn btn-primary"
                  style={{ width: '100%', justifyContent: 'center', gap: 8, padding: '10px 24px', whiteSpace: 'nowrap' }}
                >
            ...
            {/* Error / Success — full width below inputs */}
            {(formError || formSuccess) && (
              <div style={{ marginTop: 14 }}>
                {formError && (
                  <div style={{ background: 'rgba(183,28,28,0.08)', color: '#7B1C1C', padding: '10px 14px', borderRadius: 8, fontSize: 12, display: 'flex', gap: 8, alignItems: 'flex-start', border: '1px solid rgba(183,28,28,0.2)' }}>
                    <AlertCircle size={14} style={{ flexShrink: 0, marginTop: 1 }} /> {formError}
                  </div>
                )}
                {formSuccess && (
                  <div style={{ background: 'rgba(46,125,50,0.08)', color: '#2E7D32', padding: '10px 14px', borderRadius: 8, fontSize: 12, border: '1px solid rgba(46,125,50,0.2)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
          ...
          <table>
            ...
            <tbody>
              {activeLoans.length === 0 ? (
                ...
                const book      = books.find(b => b.id === l.bookId || b.no_induk === l.bookCode || b.copies?.some(c => c.copy_code === l.bookCode));
                ...
                return (
                  <tr
                    ...
                    style={{ cursor: 'pointer', background: isLate ? 'rgba(183,28,28,0.025)' : idx % 2 === 0 ? 'white' : '#fafafa' }}
                  >
                    <td style={{ width: 52 }}>
                    ...
                    <td style={{ fontSize: 12, color: isLate ? '#B71C1C' : '#374151', fontWeight: isLate ? 700 : 400, whiteSpace: 'nowrap' }}>
        ...
        <div style={{ padding: '10px 18px', fontSize: 12, color: 'var(--gray-text)', borderTop: '1px solid #f5f0ee' }}>
          {activeLoans.length} pinjaman ditampilkan
        </div>
      </div>
    </div>
  );
