...
const DISCIPLINE_META = {
  Matematika: { color: '#7B1C1C', bg: 'rgba(123,28,28,0.08)', label: 'MTK' },
  Fisika: { color: '#0D1B2A', bg: 'rgba(13,27,42,0.08)', label: 'FIS' },
  Kimia: { color: '#1B5E20', bg: 'rgba(27,94,32,0.08)', label: 'KIM' },
  Biologi: { color: '#1A237E', bg: 'rgba(26,35,126,0.08)', label: 'BIO' },
};
...
const getPrefix = (no_klasifikasi, category) => {
  ...
  if (mapKode[kode]) return mapKode[kode];
  ...
  return mapCategory[category] || 'BK';
};

const getCoverColor = (book) => {
  const prefix = getPrefix(book.no_klasifikasi, book.category);
  return CODE_COLORS[prefix] || '#555';
};
...
function StatusBadge({ available, size = 'sm' }) {
  ...
  if (available === 0) {
    return (
      <span className="badge badge-danger" style={{ padding: pad, fontSize: fs, gap: 4 }}>
        <XCircle size={fs} />Tidak Tersedia
      </span>
    );
  }
  ...
  if (available <= 1) {
    return (
      <span className="badge badge-warning" style={{ padding: pad, fontSize: fs, gap: 4 }}>
        <Clock size={fs} />Terbatas
      </span>
    );
  }

  return (
    <span className="badge badge-success" style={{ padding: pad, fontSize: fs, gap: 4 }}>
      <CheckCircle size={fs} />Tersedia
    </span>
  );
}
...
function BookCard({ book, onClick }) {
  ...
  const prefix = getPrefix(book.no_klasifikasi, book.category);
  const meta = DISCIPLINE_META[book.category] || {};
      ...
      <div style={{ padding: '14px 14px 16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 2 }}>
          <span style={{
            ...
            color: meta.color || 'var(--gray-text)',
...
/>
      ...
      <td style={{
        fontWeight: 700,
        color: book.available === 0 ? 'var(--danger)' : 'var(--success)'
      }}>
        {book.available}
      </td>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 520, padding: 0, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
        <div style={{
          background: book.image_url ? '#111' : `linear-gradient(160deg, ${coverColor} 0%, ${coverColor}bb 100%)`,
...
/>
        ...
        <div style={{ padding: 24 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 16 }}>
            {[
              ['No. Induk', <code key="c" style={{ background: 'var(--gray-light)', padding: '2px 6px', borderRadius: 4, fontSize: 11 }}>{book.no_induk}</code>],
              ['Klasifikasi', book.no_klasifikasi || '—'],
...
export default function MahasiswaPage() {
  ...
  const filtered = useMemo(() => (books || []).filter(b => {
    ...
    const matchSearch = !q
      ...
      || b.no_induk?.toLowerCase().includes(q)
      || b.no_klasifikasi?.toLowerCase().includes(q);

    const matchDis = discipline === 'Semua Disiplin' || b.category === discipline;
  ...
  const totalBorrowed = (books || []).reduce((s, b) => s + (Number(b.stock || 0) - Number(b.available || 0)), 0);
